...
import { PrismaClient, AssetState, ReportStatus, TicketStatus, User } from '@prisma/client';
...
export async function seedDemoData(prisma: PrismaClient) {
    console.log('🌱 Starting robust demo data seed...');

    // --- 1. USERS & CREW ---
    console.log('Creating Users...');
    // Use a fixed salt or just hash it now to ensure it matches the library version
    // We use a dynamic import effectively or just rely on bcryptjs being present
    // For simplicity in this shared module, we assume bcryptjs is available or use a simple mock if needed for pure demo speed
    // But strictly, we should use the same hashing as the app.
    ...
    for (const asset of assetsData) {
        // Only create if not exists, but we don't have a unique key other than ID (which we don't know). 
        // We'll trust name or serial helper or just create if we can ensure uniqueness.
        // Ideally asset serial should be unique in schema, but strict upsert on non-unique fields is tricky.
        // For demo seed, we can check.
        const existing = await prisma.asset.findFirst({ where: { serialNumber: asset.serial } });
        let a;
        if (existing) {
            a = existing;
        } else {
        ...
        // Create Maintenance History (only if new or sparse, but for now just skip if exists to avoid dupes on re-reun)
        const logs = await prisma.maintenanceLog.count({ where: { assetId: a.id } });
        if (logs === 0) {
    ...
    let fiberProject = await prisma.project.findFirst({ where: { name: 'Fiber Expansion Phase 1' } });
    if (!fiberProject) {
        fiberProject = await prisma.project.create({
            data: {
    ...
    let riverProject = await prisma.project.findFirst({ where: { name: 'Mississippi River Crossing' } });
    if (!riverProject) {
        riverProject = await prisma.project.create({
            data: {
    ...
    let resiProject = await prisma.project.findFirst({ where: { name: 'Lakeside Drive Utilities' } });
    if (!resiProject) {
        resiProject = await prisma.project.create({
            data: {
    ...
    // Geotech
    const existingGeotech = await prisma.geotechReport.findFirst({ where: { projectId: fiberProject.id } });
    if (!existingGeotech) {
    ...
    // Bores
    let bore1 = await prisma.bore.findFirst({ where: { projectId: fiberProject.id, name: 'Bore A-1 (Main St Crossing)' } });
    if (!bore1) {
        bore1 = await prisma.bore.create({
            data: {
                projectId: fiberProject.id,
        ...
        // --- 9. ROD PLANS (Moved here as it depends on bore1) ---
    ...
    // Check if reports exist to avoid dupes
    const reportCount = await prisma.dailyReport.count({ where: { projectId: fiberProject.id } });
    if (reportCount === 0) {
        for (let i = 0; i < 10; i++) {
            const date = new Date();
            ...
            if (!isToday && inventoryItems.length > 0) {
    ...
    // Safety Data - JSAs
    const jsaCount = await prisma.jSA.count({ where: { projectId: fiberProject.id } });
    if (jsaCount === 0) {
        for (let i = 0; i < 5; i++) {
    ...
    // Toolbox Talks
    const safetyCount = await prisma.safetyMeeting.count({ where: { projectId: fiberProject.id } });
    if (safetyCount === 0) {
    ...
    // Inspections
    const inspCount = await prisma.inspection.count({ where: { projectId: fiberProject.id } });
    if (inspCount === 0 && assets.length > 3) {
    ...
    // QC Punch List
    const punchCount = await prisma.punchItem.count({ where: { projectId: fiberProject.id } });
    if (punchCount === 0) {
    ...
    // --- 7. RIVER PROJECT DETAILS ---
    console.log('Populating River Project...');

    // River Obstacles
    const obsCount = await prisma.obstacle.count({ where: { projectId: riverProject.id } });
    if (obsCount === 0) {
    ...
    // River Bore
    const riverBore = await prisma.bore.findFirst({ where: { projectId: riverProject.id, name: 'River Crossing Main' } });
    if (!riverBore) {
    ...
    const expCount = await prisma.expense.count({ where: { projectId: fiberProject.id } });
    if (expCount === 0) {
    ...
    await prisma.ticket811.upsert({
        ...
        create: {
            ...
            expirationDate: faker.date.soon({ days: 2 }),
            ...
            responses: {
                create: [
                    ...
                    { utilityName: 'Comcast', status: 'Not Responded' }
                ]
            }
        }
    });
    ...
    await prisma.ticket811.upsert({
        ...
        create: {
            ...
            expirationDate: faker.date.recent({ days: 5 }),
            company: 'Midwest Underground',
